---
title: 'One Week Challenge: Building a Blog Website with Next.js'
...
<p className="lead">
  During a week-long break from my role as a software architect, I embarked on a
  personal project: replacing my outdated Jekyll website with a modern blog. In
  this post, I'll guide you through building a feature-rich blog using Next.js,
  Contentlayer, MDX, Tailwind CSS, and TypeScript, in under a week.
</p>
...
Initially, as someone with a background in using [Sass](https://sass-lang.com) for writing CSS, I was hesitant to adopt [Tailwind CSS](https://tailwindcss.com) due to its seemingly never-ending `class` attributes and lack of semantic class names.
...
1. **Utility-first approach:** Tailwind CSS provides a utility-first approach with a comprehensive set of classes. It's easy to look past long `class` attributes when you're able build this quickly.

2. **Effortless variations:** Built-in media query utilities in Tailwind CSS enable easy adjustments for different screen sizes and colour themes.

3. **Design system:** The included colour palette, spacing options, and screen size media queries promote consistent and considered designs.

4. **Tooling:** Tailwind CSS offers an ecosystem of powerful plugins and extensions, such as the [@tailwindcss/typography](https://tailwindcss.com/docs/typography-plugin) plugin, the [Prettier plugin](https://github.com/tailwindlabs/prettier-plugin-tailwindcss), and the [VSCode extension](https://marketplace.visualstudio.com/items?itemName=bradlc.vscode-tailwindcss).

5. **Performance:** Tailwind CSS optimises stylesheets file sizes by generating only the necessary CSS based on the classes you actually use.

Overall, Tailwind CSS proved to be an excellent choice for building a modern, responsive, and performant blog website, while also streamlining the development process.
